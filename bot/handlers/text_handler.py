    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            if intent == "buscar_contratos":
                ...
                id_contrato = params.get("id_contrato", "").strip().upper().replace(" ", "")
                ...
                if depto:
                    ...
                    for a in high_risk:
                        try:
                            ...
                            c_data = c_resp.json()
                            contract_dept = c_data.get("contract", {}).get("departamento", "")
                            if depto.lower() in contract_dept.lower():
                                a["departamento"] = contract_dept
                                filtered.append(a)
                        except Exception:
                            pass
                    high_risk = filtered
                ...
                limite = min(params.get("limite", 5), 10)
                high_risk = high_risk[:limite]

                return {
                    "total_alto_riesgo": len(audits),
                    "contratos": high_risk,
                }
